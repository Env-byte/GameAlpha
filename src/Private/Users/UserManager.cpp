...
unique_ptr<User> UserManager::GetUser(const string &name) {
    ...
    MYSQL_ROW row;
    stringstream sql;
    sql << "SELECT id,name,level,bossNo,stats FROM users WHERE name='" << name << "';";
...
UserList UserManager::GetAll() {
    ...
    if (mysql_query(db, "SELECT id,name,level,bossNo,stats FROM `users`")) {
...
unique_ptr<User> UserManager::GetUserById(const int &id) {
    ...
    MYSQL_ROW row;
    stringstream sql;
    sql << "SELECT id,name,level,bossNo,stats FROM users WHERE id='" << id << "';";
    if (!mysql_query(db, sql.str().c_str())) {
        rset = mysql_use_result(db);
        row = mysql_fetch_row(rset);
        if (row) {
